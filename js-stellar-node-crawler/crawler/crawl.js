...
const { NodeConfig,createCrawler, createCrawlFactory } = require('../lib');
...
//const {  NodeConfig} = require('../lib/node-configuration');
...
async function main() {
	...
	try {
		...
		await jsonStorage.writeFilePromise(
                        './crawl_result/all_nodes.json',
                        JSON.stringify(Array.from(crawl.crawledNodeAddresses))
                );
		await jsonStorage.writeFilePromise(
                        './crawl_result/crawldata.json',
                        JSON.stringify(Array.from(result.latestClosedLedger))
                );
